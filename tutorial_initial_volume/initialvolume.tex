...
The first step is to import the tilted pairs micrographs to your
scipion project. To do this, select the \menu{Import micrographs
  pairs} protocol. In \textit{Pattern untilted} and \textit{Pattern
  tilted} you must indicate where your untilted and tilted micrograph
files are stored (click on the \keys{browse} button, the one that
looks like a folder).
...
Once the protocol has finished you can press on the \keys{Analyze Results} button and a new window will open
where you can see the imported micrograph pairs, as shown in figure \ref{ImportResults}.
...
Go to the \menu{Picking micrographs pairs} protocol  and a form as shown in figure~\ref{PickingProtocol1} will appear. Select the Micrographs Tilt Pair object produced by the import protocol as input (see figure~\ref{PickingProtocol2}) and click on the \keys{Execute} button. The Xmipp particle picking GUI will open.
...
\caption{Picking pairs protocol selection object}
