...
\noindent
We use \comm{with-all-packages} to install the packages supported, eg:
Relion, Eman, Frealign, Spider, etc; We can also use local installations
of specific packages by providing a path to \comm{--with-package} option.
For example, we have specified the Xmipp location passing \verb+$XMIPP_HOME+ as argument.
