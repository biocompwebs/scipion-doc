...
\item[CTFFind] \hfill \\
To estimate the CTF with CTFFind, you will need
some parameters describing the frequency region to be analyzed. The parameters
shown in figure (\ref{fig:CTFFind}) are the proper ones for this example.
The limiting frequencies must be such that all zeros of
the PSD are contained within those frequencies. There is a wizard, shown
in figure (\ref{fig:CTFwizard}), that helps in choosing those frequencies.
Please note that the range of
defocus search is usually larger than the one used in this example, and it must
be chosen according to your data set.

\item[Xmipp CTF estimation] \hfill \\
If you prefer, you can estimate the CTF with
