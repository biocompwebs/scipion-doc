...
To run projection matching protocol click on
...
\item \textit{Resolution limit for grouping(A):} 10.  To correct CTF we need to specify maximum resolution to consider CTF differences among different groups of particles. 
...
\textbf{Analyze Results} button. Protocol viewer will be displayed (Figure \ref{fig:ProjMatchViewer}).
...
In the field 
\textit{Display volume with slices } Press \textbf {eye} button and you will see the 
volume slice by slice (Figure \ref{fig:volShowj}). To visualize volume with chimera select
...
Protocol input parameters are displayed in Figures \ref{fig:FrealignA}, \ref{fig:FrealignB}, \ref{fig:FrealignC}
and \ref{fig:FrealignD} . Here we explain only the ones we need to update
for this refinement. For more detail on each parameter please use the respective 
\textbf{help} button.
...
We need to specify:
\begin{itemize}
\item \textit{Input particles:} Set of particles generated previously.
\item \textit{Initial 3D reference volumes:}  Projection matching output volume
\item \textit{Reconstruction radius:} Inner, 10. Outer, 315.
\item \textit{Point group symmetry:} I1.
\item \textit{Resolution of reconstruction:} 15. 
\item \textit{Resolution in refinement:} Low, 200. High, 20. 
\item \textit{High resolution for classification:} 20. 
